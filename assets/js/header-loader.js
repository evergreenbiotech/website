document.addEventListener("DOMContentLoaded", function () {
  ...
  fetch(headerFile)
    ...
    .then((html) => {
      ...
      injectDynamicStyles(); // spacing, sizing, colors, etc.
  ...
  function buildMobileMenu() {
    ...
    navButton.innerHTML =
      '<a href="#navPanel" class="toggle" role="button" aria-label="Open Menu"></a>';
    document.body.appendChild(navButton);

    // Panel (width comes from injected CSS var --nav-panel-w)
    const panel = document.createElement("div");
    panel.id = "navPanel";
    panel.innerHTML =
      '<nav class="panel-nav"><ul class="panel-list"></ul></nav>';
    ...
    if (desktopNav) {
      const allLinks = desktopNav.querySelectorAll(
        'a[href]:not(#languageToggleButton)'
      );
      allLinks.forEach((a) => {
    ...
    const openMenu = () => {
      ...
      // Lock background scroll to avoid odd touch behavior
    ...
    // Close on outside tap/click – single interaction
    const closeOnce = (e) => {
      e.preventDefault();
      e.stopPropagation();
      closeMenu();
  ...
  // ------------------ Small style tweaks (width defined here) ------------------
  function injectDynamicStyles() {
    ...
    s.textContent = `
      ...
      /* Width (adjust here if you want a different drawer width) */
      ...
      /* ── Stronger spacing for ALL depth-1 items (prevents clustering) ── */
      #navPanel .panel-list li.depth-1-item { margin: 8px 0; }
